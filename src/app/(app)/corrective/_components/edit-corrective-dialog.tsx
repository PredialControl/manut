...
import { updateCorrectiveCall } from "../_actions/update-corrective-call";
import { getContractStructure } from "../_actions/get-contract-structure";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

interface EditCorrectiveDialogProps {
  open: boolean;
  onClose: () => void;
  contractId: string;
  correctiveCall: {
...
export function EditCorrectiveDialog({ open, onClose, contractId, correctiveCall }: EditCorrectiveDialogProps) {
  ...
  // Preencher dados iniciais quando os dados carregarem
  useEffect(() => {
    if (correctiveCall && assets.length > 0) {
      if (correctiveCall.assetId) {
        setSelectedAsset(correctiveCall.assetId);
      }
      if (correctiveCall.locationName) {
        setCustomLocation(correctiveCall.locationName);
      }
    }
  }, [correctiveCall, assets]);
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const result = await updateCorrectiveCall(formData);
      
      if (result.success) {
        toast.success("Corretiva atualizada com sucesso!");
        onClose();
        router.refresh();
      } else {
        toast.error(result.error || "Erro ao atualizar corretiva");
      }
    } catch (error) {
      toast.error("Erro ao atualizar corretiva");
        ...
        <form action={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              defaultValue={correctiveCall.description}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Select name="priority" defaultValue={correctiveCall.priority}>
